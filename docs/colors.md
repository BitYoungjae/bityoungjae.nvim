...
| **float**      | `#2B2B32` | -       | 플로트 표면 (Void 위 1.4:1, 투명 모드의 온릭스 위에서도 1.25:1) |
| **float_border** | `#62626C` | -     | 플로트 단면 라인 (표면 대비 ~2.4:1 — 카드 윤곽) |
...
- **플로트**: 표면 `#2B2B32`·단면 `#62626C`. snacks 픽커는 `SnacksPicker*` 그룹을 직접 정의해 투명 모드에서도 카드가 유지된다 (픽커 현재 행은 Visual을 빌려 쓰므로 표면과 1.2:1 + hue로 구분).
